...
for set_name in sets:
    ...
    for image_name in image_list:
        ...
        display_base_name = base_name[:1].upper() + base_name[1:]
        ...
        cv2.imwrite(os.path.join(interim_path, f"{display_base_name}_A_WeldGapPosition.jpg"), roi_vis)
        cv2.imwrite(os.path.join(interim_path, f"{display_base_name}_B_InterimResult1.jpg"), sobel_img)

        # =========================
        # STORE RESULT (NO SPACES)
        # =========================
        results.append(f"{display_base_name}.jpg,{x_position},{valid}")

        print(f"{set_name} | {display_base_name}.jpg → x: {x_position}, valid: {valid}, score: {best_score:.2f}")

    # =========================
    # SAVE CSV (INSIDE INTERIM FOLDER)
    # =========================
    ...
    print(f"{set_name} complete. CSV saved to {csv_path}")
